...
export function OgPreview({document: {displayed}, options}: OgPreviewProps) {
  ...
  const title = (displayed?.seoTitle as string) || (displayed?.title as string) || (displayed?.name as string) || ''
  const description = (displayed?.seoDescription as string) || ''
  const slug = getSlug(displayed)

  return (
    <div style={{padding: '24px', maxWidth: '720px'}}>
      <h2 style={{fontSize: '16px', fontWeight: 600, marginBottom: '16px', color: '#27272a'}}>
        Social Share Preview
      </h2>

      {!slug && options.type !== 'conference' && (
        <div style={{padding: '16px', backgroundColor: '#fefce8', borderRadius: '6px', fontSize: '14px', color: '#854d0e', marginBottom: '16px'}}>
          Add a slug to see the OG image preview.
        </div>
      )}

      {error && (
        <div style={{padding: '12px 16px', backgroundColor: '#fef2f2', borderRadius: '6px', fontSize: '14px', color: '#dc2626', marginBottom: '16px'}}>
          {error}
        </div>
      )}
      ...
      >
        {imageUrl ? (
          <img
            src={imageUrl}
            alt="OG preview"
            style={{width: '100%', height: 'auto', display: 'block', aspectRatio: '1200 / 630'}}
          />
        ) : (
          <div style={{aspectRatio: '1200 / 630', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a1a1aa', fontSize: '14px'}}>
        ...
        <div style={{padding: '12px 16px', borderTop: '1px solid #e4e4e7'}}>
          <p style={{fontSize: '12px', color: '#a1a1aa', margin: 0}}>
            everything-nyc.sanity.dev
          </p>
          {title && (
            <p style={{fontSize: '14px', fontWeight: 600, color: '#27272a', margin: '4px 0 0'}}>
              {title}
            </p>
          )}
          {description && (
            <p style={{fontSize: '13px', color: '#71717a', margin: '2px 0 0', lineHeight: 1.4}}>
      ...
      {/* Character counts */}
      <div style={{marginTop: '16px', display: 'flex', gap: '24px', fontSize: '12px', color: '#a1a1aa'}}>
        <span>
          Title: {title.length}/60
          {title.length > 60 && <span style={{color: '#f59e0b'}}> (may be truncated)</span>}
        </span>
        <span>
          Description: {description.length}/160
          {description.length > 160 && <span style={{color: '#f59e0b'}}> (may be truncated)</span>}
        </span>
      </div>
    </div>
  )
